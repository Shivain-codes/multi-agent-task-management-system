...
def _get_calendar_service():
    ...
    # Cloud Run-safe path: use service account credentials when configured.
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if settings.app_env == "production":
                raise RuntimeError(
                    "Interactive OAuth is disabled in production. "
                    "Set GOOGLE_SERVICE_ACCOUNT_PATH to a mounted credentials file."
                )
